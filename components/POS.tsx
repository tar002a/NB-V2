...
import { Search, Plus, Minus, Trash2, ShoppingCart, CreditCard, ShoppingBag, ArrowLeft, Copy, Instagram, Check, X, Edit2, UserCheck, User, Filter, Clock, Printer, ShoppingBasket, Truck } from 'lucide-react';
...
const POS: React.FC<POSProps> = ({ products, customers, onCompleteSale }) => {
  ...
  const copyForInstagram = () => {
    ...
    const itemsText = lastOrder.cart.map((i: any) => 
      `القطعة: ${i.name}\nاللون: ${i.color} | القياس: ${i.size}\nالعدد: ${i.quantity}\nالسعر: ${i.sellingPrice.toLocaleString()}`
    ).join('\n---\n');
    ...
    const fullText = `🌸 تم تثبيت طلبج بنجاح حبيبتي\n\n` +
                     ...
                     `🚚 مدة التوصيل: خلال ${lastOrder.customer.deliveryDuration} إن شاء الله. المندوب راح يتصل بيج قبل ما يوصل.\n\n` +
  ...
  return (
    <div className="flex flex-col md:flex-row h-full gap-4 relative overflow-hidden">
      ...
      <div className={`flex-1 flex flex-col gap-4 h-full overflow-hidden ${mobileView === 'CART' ? 'hidden md:flex' : 'flex'}`}>
        ...
        {cart.length > 0 && mobileView === 'PRODUCTS' && (
          <button 
            ...
            className="md:hidden fixed bottom-20 left-6 right-6 bg-primary text-white py-4 rounded-2xl shadow-2xl flex items-center justify-between px-6 animate-fade-in-up z-40"
          >
                        ...
                        <div className="flex justify-between items-center mt-1">
                            {editingPriceId === item.id ? (
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button 
                                        onClick={() => { setEditingPriceId(item.id); setTempPrice(item.sellingPrice.toString()); }} 
                                        className="p-1 text-gray-500 hover:text-primary transition-colors opacity-0 group-hover/item:opacity-100"
                                    >
            ...
            {isCheckoutMode ? (
                <div className="space-y-3">
                    <div className="relative">
                      ...
                      <input value={customerName} onChange={handleCustomerNameChange} placeholder="اسم العميل" className="w-full bg-dark border border-gray-700 rounded-xl py-3 pr-10 pl-4 text-white focus:border-primary outline-none" />
                      {foundCustomerMsg && <span className="absolute left-3 top-3.5 text-green-500 text-[10px] font-bold">{foundCustomerMsg}</span>}
                    </div>
                    <div className="relative">
                      <Clock className="absolute right-3 top-3.5 text-gray-500 w-4 h-4" />
                      <input value={customerPhone} onChange={e => setCustomerPhone(e.target.value.replace(/[^0-9]/g, ''))} placeholder="رقم الهاتف" className="w-full bg-dark border border-gray-700 rounded-xl py-3 pr-10 pl-4 text-white focus:border-primary outline-none" maxLength={11} />
                    </div>
                    <div className="relative">
                      ...
                      <select 
                        ...
                        className="w-full bg-dark border border-gray-700 rounded-xl py-3 pr-10 pl-4 text-white focus:border-primary outline-none appearance-none"
                      >
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={handleCheckout} className="flex-[2] bg-green-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-green-900/20 hover:bg-green-500 transition-colors">تأكيد العملية</button>
                    </div>
                </div>
            ) : (
                <div className="space-y-4">
                    <div className="flex justify-between items-end">
                        <span className="text-gray-400 text-sm">المجموع النهائي</span>
                        <div className="text-left">
                            <span className="text-2xl font-bold text-white">{cartTotal.toLocaleString()}</span>
                            <small className="text-xs text-primary mr-1">د.ع</small>
                        </div>
                    </div>
                    <button onClick={() => setIsCheckoutMode(true)} disabled={cart.length === 0} className="group w-full bg-primary disabled:bg-gray-800 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-primary/20 hover:bg-primaryHover transition-all">
                        <CreditCard className="w-5 h-5 group-hover:scale-110 transition-transform" /> استكمال الطلب
                    </button>
                </div>
            )}
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in" onClick={() => setSizePickerGroup(null)}>
            <div className="bg-[#1e1e1e] w-full max-w-sm rounded-3xl border border-gray-800 p-6 flex flex-col gap-6 animate-fade-in-up shadow-2xl" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center">
                    <div>
                        <h3 className="text-white font-bold text-xl">{sizePickerGroup.name}</h3>
                        <div className="flex items-center gap-2 mt-1">
                            <span className="w-3 h-3 rounded-full border border-white/10" style={{backgroundColor: getColorHex(sizePickerGroup.color)}}></span>
                            <span className="text-gray-400 text-sm font-bold">{sizePickerGroup.color}</span>
                        </div>
                    </div>
                    <button onClick={() => setSizePickerGroup(null)} className="text-gray-500 hover:text-white p-2 hover:bg-white/5 rounded-full"><X className="w-6 h-6" /></button>
                </div>
                
                <div className="bg-dark/50 p-4 rounded-2xl border border-border">
                    <p className="text-[10px] text-gray-500 mb-3 uppercase tracking-widest font-black">اختر المقاس المطلوب</p>
                    <div className="grid grid-cols-2 gap-3">
                        {sizePickerGroup.variants.map((v: any) => (
                            <button 
                                key={v.id} 
                                onClick={(e) => addToCart(v, e)}
                                disabled={v.stock <= 0}
                                className={`group relative py-4 rounded-2xl border text-sm font-bold transition-all flex flex-col items-center gap-1 ${v.stock <= 0 ? 'bg-gray-900/40 border-gray-800 text-gray-700 cursor-not-allowed' : 'bg-dark border-gray-700 text-white hover:border-primary hover:text-primary active:scale-95 shadow-sm'}`}
                            >
                                <span className="text-lg">{v.size}</span>
                                <span className={`text-[10px] font-normal ${v.stock <= 3 ? 'text-red-500' : 'opacity-60'}`}>{v.stock <= 0 ? 'منتهي' : `متوفر: ${v.stock}`}</span>
                            </button>
                        ))}
                    </div>
                </div>
      ...
      {/* Floating Note (Added Feedback) */}
      {addedNote && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[200] bg-green-600 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-fade-in-down">
           <Check className="w-5 h-5" />
           <span className="font-bold">{addedNote}</span>
        </div>
      )}
      ...
      {showReceipt && lastOrder && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/95 p-4 animate-fade-in" onClick={() => setShowReceipt(false)}>
          <div className="bg-[#1a1a1a] w-full max-w-md rounded-3xl border border-gray-800 shadow-2xl overflow-hidden flex flex-col animate-fade-in-up" onClick={e => e.stopPropagation()}>
            <div className="p-6 bg-green-500/10 text-center border-b border-green-500/20">
               <div className="w-12 h-12 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-3"><Check className="text-white w-6 h-6" /></div>
               <h2 className="text-xl font-black text-white">تم تأكيد الطلب بنجاح</h2>
               <p className="text-green-500 text-xs mt-1 font-bold">#{lastOrder.id}</p>
            </div>
            
            {/* Copy Section */}
            <div className="p-6 bg-[#121212]">
                <div className="flex justify-between items-center mb-4">
                    <span className="text-xs text-gray-500 font-bold uppercase tracking-wider">نص التأكيد لانستغرام</span>
                    <button 
                        onClick={copyForInstagram}
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${copiedMsg ? 'bg-green-600 text-white' : 'bg-primary/20 text-primary border border-primary/30 hover:bg-primary/30'}`}
                    >
                        {copiedMsg ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                        {copiedMsg ? 'تم النسخ' : 'نسخ النص للعميلة'}
                    </button>
                </div>
                <div className="bg-black/40 rounded-xl p-4 border border-white/5 font-mono text-[11px] text-gray-400 whitespace-pre-wrap leading-relaxed">
...
تتهنين بيها مقدماً، وشكراً لثقتج بـ نواعم بوتيك 🤍`}
            ...
            <div className="p-4 grid grid-cols-2 gap-4 bg-[#1a1a1a] border-t border-border">
                <button onClick={() => setShowReceipt(false)} className="bg-gray-800 text-white py-4 rounded-2xl font-bold">إغلاق</button>
                <button onClick={() => { setShowReceipt(false); }} className="bg-primary text-white py-4 rounded-2xl font-bold shadow-lg shadow-primary/20">فاتورة جديدة</button>
            </div>
          </div>
        </div>
      )}
